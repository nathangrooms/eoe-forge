...
(() => {
  ...
  const CONTAINER_B = 'cccccccc-0000-4000-8000-00000000dm02';
  ...
  function load() {
    ...
    /* --- REFUTE PASS: surfaces the first phase changed but never measured ---
       `storage-move`   the move panel on /collection/storage/:id
       `collection-assign`  "Assign to storage" over picked collection rows
       `price-sync`     the TCGPlayer price sync on the analytics tab       */
    if (
      ...
      const owned = cardPool.slice(0, COLLECTION || 100);
      ...
      /* Already filed, so the move panel has stacks to pick up. */
      if (SCENARIO === 'storage-move') {
        owned.forEach((card, i) => {
          TABLES.storage_items.push({
            ...
            qty: 1,
            foil: false,
            created_at: `2026-01-01T00:${String(i % 60).padStart(2, '0')}:00+00:00`,
            updated_at: '2026-01-01T00:00:00+00:00',
          });
        });
      }
    ...
    if (SCENARIO.startsWith('storage-deck') || SCENARIO === 'storage-collection') {
      TABLES.storage_containers = [
        {
          id: CONTAINER_ID,
          user_id: USER_ID,
  ...
  function rpcAnswer(name, body) {
    ...
    /* A move that actually moves, so a batch that claims to have moved fifty
       cards can be checked against fifty rows that changed. */
    function moveOne(itemId, qty, toContainer, toSlot, toPocket) {
      const source = TABLES.storage_items.find(r => String(r.id) === String(itemId));
      if (!source) throw new Error('storage row not found');
      if (!(qty > 0) || qty > source.qty) throw new Error('not that many copies here');
      const container = TABLES.storage_containers.find(c => String(c.id) === String(toContainer));
      if (!container) throw new Error('destination container not found');

      let landed = null;
      const merge =
        toPocket == null &&
        TABLES.storage_items.find(
          r =>
            String(r.container_id) === String(toContainer) &&
            String(r.card_id) === String(source.card_id) &&
            Boolean(r.foil) === Boolean(source.foil) &&
            (r.slot_id ?? null) === (toSlot ?? null) &&
            r.pocket == null &&
            String(r.id) !== String(source.id)
        );
      ...
      if (merge) {
        ...
        const made = {
          id: `si-moved-${Math.random().toString(36).slice(2, 10)}`,
          container_id: toContainer,
          slot_id: toSlot ?? null,
        ...
        TABLES.storage_items.push(made);
        landed = made.id;
      }

      source.qty -= qty;
      if (source.qty <= 0) {
        TABLES.storage_items = TABLES.storage_items.filter(r => String(r.id) !== String(source.id));
      }
      return landed;
    }

    if (name === 'storage_move_cards') {
      return moveOne(
        body && body.p_item_id,
        Number(body && body.p_qty),
        body && body.p_to_container,
        (body && body.p_to_slot) ?? null,
        (body && body.p_to_pocket) ?? null
      );
    }

    if (name === 'storage_move_cards_batch') {
      const moves = (body && body.p_moves) || [];
      if (moves.length > 500) throw new Error('storage_move_cards_batch takes at most 500 moves');
      return moves.map((move, at) => {
        try {
          const landed = moveOne(
            move.item_id,
            Number(move.qty),
            move.to_container,
            move.to_slot ?? null,
          ...
          return { at, item_id: move.item_id, moved_to: landed, error: null };
        } catch (error) {
          return { at, item_id: move.item_id, moved_to: null, error: String(error.message || error) };
        }
      });
    }

    /* An RPC this shim does not know must be LOUD. Answering null makes the
       calling code fail quietly, and a page that failed quietly reads as a
       cheap page. That is the trap this whole exercise is about. */
    window.__dmUnknownRpc = window.__dmUnknownRpc || [];
    window.__dmUnknownRpc.push(name);
    return null;
  }
  ...
  /* How many deck tiles say "Scoring…" AT ONCE, at any point in the visit.
     The deck list used to score one deck at a time and light one spinner at a
     time. A batch that lights all twelve is a change to what the screen looks
     like, which is not allowed, so it has to be watched rather than assumed.
     Sampled on animation frames: it only reads when the page could paint. */
  window.__dmMaxScoring = 0;
  (function sampleScoring() {
    try {
      let n = 0;
      for (const button of document.querySelectorAll('button')) {
        if ((button.textContent || '').includes('Scoring')) n += 1;
      }
      if (n > window.__dmMaxScoring) window.__dmMaxScoring = n;
    } catch {}
    requestAnimationFrame(sampleScoring);
  })();

  /* What the database HOLDS after the action, so "same data" is checked against
     rows rather than against a screenshot. */
  window.__dmState = () => ({
    user_collections: TABLES.user_collections
      .map(r => `${r.card_id}|${r.quantity}|${r.foil}|${r.set_code}|${r.condition}`)
      .sort(),
    storage_items: TABLES.storage_items
      .map(r => `${r.container_id}|${r.slot_id ?? '-'}|${r.pocket ?? '-'}|${r.card_id}|${r.qty}|${Boolean(r.foil)}`)
      .sort(),
    storage_slots: TABLES.storage_slots.map(r => `${r.container_id}|${r.name}|${r.position}`).sort(),
    deck_cards: TABLES.deck_cards.length,
    user_decks: TABLES.user_decks
      .map(r => `${r.name}|${r.power_level}|${JSON.stringify(r.edh_analysis && r.edh_analysis.deckmatrix ? r.edh_analysis.deckmatrix.score : null)}`)
      .sort(),
  });
  ...
  window.fetch = async function (input, init) {
    ...
    /* Scryfall is counted too. The price sync's whole cost is Scryfall calls,
       so a measurement that only counted Supabase would have said the change
       was free. Answered here rather than forwarded: the count is the thing
       being measured, and 5,000 real calls to a third party is not something
       to do to check arithmetic. */
    if (url && url.startsWith('https://api.scryfall.com/')) {
      const single = url.match(/\/cards\/([0-9a-f-]{36})$/);
      if (single) {
        record({ method, table: 'scryfall:cards/:id' });
        const card = cardCache.get(single[1]);
        return card ? json(card) : json({ object: 'error' }, 404);
      }
      if (url.includes('/cards/collection')) {
        record({ method, table: 'scryfall:cards/collection' });
        let asked = [];
        try {
          asked = (JSON.parse(opts.body || '{}').identifiers || []).map(i => i.id);
        } catch {}
        const found = asked.map(id => cardCache.get(id)).filter(Boolean);
        return json({
          object: 'list',
          data: found,
          not_found: asked.filter(id => !cardCache.has(id)).map(id => ({ id })),
        });
      }
      record({ method, table: 'scryfall:other' });
    ...
    if (isRpc) {
      ...
      try {
        ...
        return json({ message: String(error.message || error), code: 'P0001' }, 400);
      }
    }
