...
const questions = JSON.parse(fs.readFileSync(path.join(root, 'scripts/tutor-fifty.json'), 'utf8'));
const decksArg = process.argv.find(a => a.startsWith('--decks='));
const summaries = JSON.parse(
  fs.readFileSync(decksArg ? decksArg.slice('--decks='.length) : path.join(root, 'scratch/tutor-decks.json'), 'utf8')
);
...
for (const question of questions.questions) {
  ...
  let row;
  try {
    row = await ask(question);
  } catch (error) {
    /* One retry, because a socket that dies before the request is sent tells
       you nothing about Tutor. */
    await sleep(2000);
    try {
      row = await ask(question);
    } catch (again) {
      row = { id: question.id, question: question.q, error: String(again) };
    }
  }
  process.stderr.write(
    `${row.wall_clock_ms ?? '?'}ms ${row.answered_from ?? 'error'} ask=${row.routing?.ask ?? 'none'}\n`
  );
  results.push(row);
...
const outPath = path.join(root, 'scripts/tutor-fifty-answers.json');
...
const out = {
  title: 'Fifty real Magic questions, put through the deployed Tutor endpoint',
...
process.stderr.write(
  `\nwrote scripts/tutor-fifty-answers.json (${merged.length} rows, ${results.length} asked this run)\n`
);
