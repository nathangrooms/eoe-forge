...
'| Measure | Count |',
'|---|---|',
'| `new` expressions in the constructor | ' + num(c.newInSource) + ' |',
'| Of those, inside a region kept opaque, so NOT EXTRACTED | ' + num(c.newInsideOpaque) + ' (' + pct(c.newInsideOpaque, c.newInSource) + '%) |',
'| Reachable by the walker | ' + num(c.newInSource - c.newInsideOpaque) + ' |',
'| Visited by the extractor | ' + num(c.newVisited) + ' |',
'| Cards where the extractor missed a reachable one | ' + num(c.cardsWithMissedConstruction) + ' |',
'',
"So the parser's measured failure rate over " + num(N) + ' card constructors is',
'**' + pct(N - c.fidelityOk, N) + '%**. Of the ' + num(c.newInSource) + ' constructions in',
'those constructors the walker missed **' + pct(c.newInSource - c.newInsideOpaque - c.newVisited, c.newInSource) + '%** of the ones it can',
'reach, and **' + pct(c.newInsideOpaque, c.newInSource) + '%** are inside an opaque region and are not extracted at',
...
'   them is LOST.** ' + num(c.newInsideOpaque) + ' constructions sit in one and are not extracted.',
