...
const containerWidth = Number(params.get('w') || 1160);
const TILE_WIDTH = 380;

function power(score: number, band: string, bracket: number) {
  return {
    score, band, bracket,
    subscores: {}, simulation: {}, diagnostics: {},
    drivers: [], drags: [],
    legality: { ok: true, issues: [] },
    hash: 'x', scoredAt: new Date().toISOString(),
    stale: false, source: 'engine', engineVersion: 1,
  } as any;
}

function summary(
  id: string, name: string, commanderName: string, scryfallId: string,
  identity: string[], total: number, missing: number, priceUSD: number, p: any
): DeckSummary {
  return {
    id, name, format: 'commander', colors: identity, identity,
    commander: {
      name: commanderName,
      image: `https://cards.scryfall.io/large/front/${scryfallId[0]}/${scryfallId[1]}/${scryfallId}.jpg`,
    },
    counts: { total, unique: total - 4, lands: 37, creatures: 28, instants: 8, sorceries: 7, artifacts: 10, enchantments: 6, planeswalkers: 2, battles: 0 },
    curve: { bins: { '0-1': 4, '2': 12, '3': 15, '4': 12, '5': 8, '6-7': 6, '8-9': 3, '10+': 1 } },
    mana: { sources: { W: 12, U: 14, B: 11, R: 0, G: 13, C: 4 }, untappedPctByTurn: { t1: 60, t2: 75, t3: 84 } },
    legality: { ok: true, issues: [] },
    power: p,
    economy: { priceUSD, ownedPct: 0, missing },
    tags: [], updatedAt: new Date().toISOString(), favorite: true,
  };
}
...
const FIXTURES: DeckSummary[] = [
  ...
  summary('d2', 'Goblin Swarm and a Rather Long Deck Name', PICKS[1].commanders[0].name, PICKS[1].commanders[0].scryfallId, PICKS[1].ci, 100, 0, 412, power(5.6, 'mid', 2)),
...
function Precons() {
  ...
  useEffect(() => {
    fetchCommanderCards().then(setCards).catch(() => setCards(new Map()));
  }, []);

  const partnered = PRECON_INDEX.filter(e => e.commanders.length > 1).slice(0, 2);
  const chosen = [...PRECON_INDEX.slice(0, 4), ...partnered];

  const summaries = summarizePrecons(
    chosen.map(e => ({ id: e.id, name: e.name, set: e.set, filename: `${e.id}.json` }))
  );
  ...
  return (
    <section className="space-y-4">
      ...
      <CardGrid width={TILE_WIDTH}>
        {[0, 1].map(i => <PreconTileSkeleton key={i} />)}
      </CardGrid>
    </section>
  );
}
  ...
  return (
    <BrowserRouter>
      <div className="min-h-screen bg-background p-8 text-foreground">
        <div className="mx-auto space-y-12" style={{ maxWidth: containerWidth }}>
          {only !== 'fav' && <Precons />}
          {only !== 'precon' && (
            <section className="space-y-4">
