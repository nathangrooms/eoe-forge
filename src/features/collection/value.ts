...
/*
 * The two rules that decide what a stack is worth and whether we could work it
 * out live in `priceable.ts`, which imports nothing so it can be tested: the
 * test runner is `node --test --experimental-strip-types` and does not resolve
 * the `@/` alias this file uses. They are re-exported here because every caller
 * already reaches for them at this path.
 */
export { canPriceOwnedCopies, ownedValueUSD } from './priceable';
