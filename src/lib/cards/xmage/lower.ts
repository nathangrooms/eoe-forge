...
export const REFUSED_ABILITY_CLASSES: Record<PrimId, string> = {
  ...
  'xmage:ForecastAbility':
    '11 cards, 3 of them already shipped. CR 702.56: a forecast ability is activated from the HAND, only during its owner\'s upkeep, only once each turn, and revealing the card is part of the cost. The class sets all four in its own constructor body from literals. The record holds a class name and its ARGUMENTS, so it can see none of them, and the generic path produced an ability with the mana cost alone. Steeling Stance became "{W}: Target creature gets +1/+1 until end of turn", at any time, as often as the player liked. `Cost` has no member for revealing a card from hand, so this cannot be mapped either.',
  'xmage:BoastAbility':
