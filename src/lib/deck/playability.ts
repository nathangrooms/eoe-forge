...
function coloursToMask(colours: readonly string[] | null | undefined): number {
  let mask = 0;
...
export function buildManaProfile(deck: readonly PlayabilityCardInput[]): ManaProfile {
  ...
  for (const card of deck) {
    ...
    const type = frontFace(card.type_line) ?? '';
    // Instants and sorceries that "add" mana are rituals, not sources: they
    // are one-shot and cost a card, so counting them as a permanent source
    // would inflate every deck running Dark Ritual.
    if (/\b(Instant|Sorcery)\b/i.test(type)) continue;
    ...
    const onlineTurn = onlineTurnFor(kind, mv, production.amount);
    for (let i = 0; i < qty; i++) {
      sources.push({
        name: card.name,
