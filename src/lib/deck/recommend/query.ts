...
export class UnindexedFormatError extends Error {
  ...
  constructor(format: string) {
    super(
      `No index supports format "${format}". Querying it would sequentially scan ` +
        `all rows of cards and risks a statement timeout (57014). Indexed formats: ` +
        `${[...INDEXED_FORMATS].sort().join(', ')}.`
    );
 ...
 *
 * @throws {UnindexedFormatError} when the format has no supporting index.
 */
export function buildCandidateQuery(profile: DeckProfile): CandidateQuery {
  const format = profile.format.toLowerCase();
  if (!INDEXED_FORMATS.has(format)) throw new UnindexedFormatError(format);
