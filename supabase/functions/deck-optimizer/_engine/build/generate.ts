...
export function generateDeck(input: GenerateDeckInput): GeneratedDeck {
  ...
  const rankedLands = rankCandidates(landPool, seedForLands, { preferBudget: budgetPressure(input) });
  ...
  const rankedSpells = rankCandidates(spellPool, roleProfile, {
    preferBudget: budgetPressure(input),
  });
  ...
  if (typeof input.budgetUsd === 'number' && input.budgetUsd > 0) {
    const swapped = trimToBudget(picked, shortlist, takenOracleIds, input.budgetUsd, chosenLands.length);
    if (swapped > 0) notes.push(`${swapped} card${swapped === 1 ? '' : 's'} swapped down to stay inside $${input.budgetUsd}`);
  }
...
function budgetPressure(input: GenerateDeckInput): boolean {
  return typeof input.budgetUsd === 'number' && input.budgetUsd > 0;
}
 ...
 *
 * The most expensive replaceable card is exchanged for the best-ranked unused
 * card that is genuinely cheaper. Lands, basics and anything already cheap are
 * left alone. Bounded, and it gives up rather than looping: a $2,000 pool with
 * a $50 budget is a fact about the request, not something to keep grinding at.
 */
function trimToBudget(
  picked: GeneratedEntry[],
  shortlist: readonly BuildCard[],
  taken: Set<string>,
  budgetUsd: number,
  landOffset: number
): number {
  ...
  for (let guard = 0; guard < 60; guard++) {
    const total = picked.reduce((n, e) => n + (e.card.usd ?? 0) * e.quantity, 0);
    if (total <= budgetUsd) break;

    let worstIndex = -1;
    let worstPrice = 0;
    for (let i = landOffset; i < picked.length; i++) {
      const entry = picked[i];
      if (entry.bucket === 'basic' || entry.preferred) continue;
      const price = entry.card.usd ?? 0;
      if (price > worstPrice) {
        worstPrice = price;
        worstIndex = i;
      }
    ...
    const replacement = shortlist.find(
      c => !taken.has(c.oracleId) && (c.usd ?? 0) < worstPrice && !isLandCandidate(c)
    );
    ...
    picked[worstIndex] = {
      card: replacement,
      quantity: 1,
      reason: `${replacement.name} at $${(replacement.usd ?? 0).toFixed(2)} in place of ${removed.card.name} at $${worstPrice.toFixed(2)}, to stay inside the budget.`,
      score: removed.score,
