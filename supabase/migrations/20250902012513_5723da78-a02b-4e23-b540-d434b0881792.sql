-- Fix the compute_deck_summary function with correct JSONB operators
...
BEGIN
    ...
    -- Calculate mana sources (fixed JSONB array operators)
    SELECT jsonb_build_object(
        'W', COALESCE(SUM(CASE WHEN c.colors @> '["W"]'::jsonb OR LOWER(c.type_line) LIKE '%plains%' THEN dc.quantity ELSE 0 END), 0),
        'U', COALESCE(SUM(CASE WHEN c.colors @> '["U"]'::jsonb OR LOWER(c.type_line) LIKE '%island%' THEN dc.quantity ELSE 0 END), 0),
        'B', COALESCE(SUM(CASE WHEN c.colors @> '["B"]'::jsonb OR LOWER(c.type_line) LIKE '%swamp%' THEN dc.quantity ELSE 0 END), 0),
        'R', COALESCE(SUM(CASE WHEN c.colors @> '["R"]'::jsonb OR LOWER(c.type_line) LIKE '%mountain%' THEN dc.quantity ELSE 0 END), 0),
        'G', COALESCE(SUM(CASE WHEN c.colors @> '["G"]'::jsonb OR LOWER(c.type_line) LIKE '%forest%' THEN dc.quantity ELSE 0 END), 0),
        'C', COALESCE(SUM(CASE WHEN (c.colors IS NULL OR c.colors = '[]'::jsonb) AND LOWER(c.type_line) LIKE '%land%' THEN dc.quantity ELSE 0 END), 0)
    ) INTO mana_sources
    FROM deck_cards dc
    LEFT JOIN cards c ON c.id = dc.card_id
    WHERE dc.deck_id = compute_deck_summary.deck_id;
