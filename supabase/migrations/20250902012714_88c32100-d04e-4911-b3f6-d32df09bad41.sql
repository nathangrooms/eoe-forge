-- Fix the compute_deck_summary function with correct text array operators
...
BEGIN
    ...
    -- Calculate mana sources (using correct text array operators)
    SELECT jsonb_build_object(
        'W', COALESCE(SUM(CASE WHEN c.colors @> ARRAY['W'] OR LOWER(c.type_line) LIKE '%plains%' THEN dc.quantity ELSE 0 END), 0),
        'U', COALESCE(SUM(CASE WHEN c.colors @> ARRAY['U'] OR LOWER(c.type_line) LIKE '%island%' THEN dc.quantity ELSE 0 END), 0),
        'B', COALESCE(SUM(CASE WHEN c.colors @> ARRAY['B'] OR LOWER(c.type_line) LIKE '%swamp%' THEN dc.quantity ELSE 0 END), 0),
        'R', COALESCE(SUM(CASE WHEN c.colors @> ARRAY['R'] OR LOWER(c.type_line) LIKE '%mountain%' THEN dc.quantity ELSE 0 END), 0),
        'G', COALESCE(SUM(CASE WHEN c.colors @> ARRAY['G'] OR LOWER(c.type_line) LIKE '%forest%' THEN dc.quantity ELSE 0 END), 0),
        'C', COALESCE(SUM(CASE WHEN (c.colors IS NULL OR array_length(c.colors, 1) IS NULL) AND LOWER(c.type_line) LIKE '%land%' THEN dc.quantity ELSE 0 END), 0)
    ) INTO mana_sources
    FROM deck_cards dc
    LEFT JOIN cards c ON c.id = dc.card_id
    WHERE dc.deck_id = compute_deck_summary.deck_id;
