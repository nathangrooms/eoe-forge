-- Fix the compute_deck_summary function to properly handle commander and card counts
...
BEGIN
    ...
    -- Calculate mana sources correctly (count lands by basic type, not multiply by colors)
    SELECT jsonb_build_object(
        'W', COALESCE(SUM(CASE 
