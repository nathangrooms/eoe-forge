-- Update compute_deck_summary to use EDH power level from edh_analysis when available
...
BEGIN
    ...
    -- Extract EDH power level from cached analysis if available, otherwise use deck power_level
    edh_power_level := COALESCE(
        (deck_record.edh_analysis->'metrics'->>'powerLevel')::numeric,
        deck_record.power_level
    );
    
    -- Determine power band based on EDH power level
    power_band := CASE 
