...
begin
  ...
  if (v_text ~ '(?:(?:(^|\n)add (\{|one |two |three |four |x ))|(?:search your library for [^\n]{0,70}land[^\n]{0,90}onto the battlefield)|(?:you may play an additional land)|(?:play an additional land)|(?:creates? [^\n]{0,60}treasure token))'
  ...
  -- The sacrifice must sit in an activated-ability COST — left of a colon on its own line. "Sacrifice a creature. If you do..." is a one-shot effect, not an outlet, and has no colon.
  if v_text ~ '(?:(?:(^|\n)[^\n:]{0,70}sacrifice (a|an|another|one|two|three|x|\d+|this|it|any number of)[^\n:]{0,60}:)|(?:(^|\n)[^\n:]{0,70}sacrifice (a|an|another)[^\n:]{0,60}:))'
  then v_tags := v_tags || array['sacrifice-outlet', 'sac-outlet', 'sac_outlet', 'sacrifice']::text[]; end if;
