-- COMPILER VERSION 16, 3 Sep 2026. `eff:bounce-own` no longer counts a
-- NON-CREATURE returning itself. Rancor (829), Batterskull and Spine of Ish
-- Sah were taking the slots Chulane's "bounce your own creatures to cast them
-- again" job wanted; a creature returning itself (Whitemane Lion) still counts,
-- because its own re-entry IS the recast.
--
-- Writer first (done), readers second (this), 15 deleted last.
...
as $$
  ...
  where m.oracle_id = c.oracle_id and m.compiler_version = 16;
$$;
...
delete from public.card_facet_memo where compiler_version in (14, 15);;
